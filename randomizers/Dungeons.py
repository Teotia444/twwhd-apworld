...
class Dungeon:
    """
    This class represents a dungeon in The Wind Waker HD, including its dungeon items.
...
def create_dungeons(world: "TWWHDWorld") -> None:
    """
    ...
    if options.progression_dungeons:
        if not options.required_bosses or "Dragon Roost Cavern" in world.boss_reqs.required_dungeons:
            world.dungeons["Dragon Roost Cavern"] = make_dungeon(
                "Dragon Roost Cavern",
                item_factory("Dragon Roost Cavern Big Key", world),
                item_factory(["Dragon Roost Cavern Small Key"] * 4, world),
                item_factory(["Dragon Roost Cavern Dungeon Map", "Dragon Roost Cavern Compass"], world),
            )
        ...
        if not options.required_bosses or "Forbidden Woods" in world.boss_reqs.required_dungeons:
            world.dungeons["Forbidden Woods"] = make_dungeon(
                "Forbidden Woods",
                item_factory("Forbidden Woods Big Key", world),
                item_factory(["Forbidden Woods Small Key"] * 1, world),
                item_factory(["Forbidden Woods Dungeon Map", "Forbidden Woods Compass"], world),
            )
        ...
        if not options.required_bosses or "Tower of the Gods" in world.boss_reqs.required_dungeons:
            world.dungeons["Tower of the Gods"] = make_dungeon(
                "Tower of the Gods",
                item_factory("Tower of the Gods Big Key", world),
                item_factory(["Tower of the Gods Small Key"] * 2, world),
                item_factory(["Tower of the Gods Dungeon Map", "Tower of the Gods Compass"], world),
            )
        ...
        if not options.required_bosses or "Forsaken Fortress" in world.boss_reqs.required_dungeons:
            world.dungeons["Forsaken Fortress"] = make_dungeon(
                ...
                None,
                [],
                item_factory(["Forsaken Fortress Dungeon Map", "Forsaken Fortress Compass"], world),
            )
        ...
        if not options.required_bosses or "Earth Temple" in world.boss_reqs.required_dungeons:
            world.dungeons["Earth Temple"] = make_dungeon(
                "Earth Temple",
                item_factory("Earth Temple Big Key", world),
                item_factory(["Earth Temple Small Key"] * 3, world),
                item_factory(["Earth Temple Dungeon Map", "Earth Temple Compass"], world),
            )
        ...
        if not options.required_bosses or "Wind Temple" in world.boss_reqs.required_dungeons:
            world.dungeons["Wind Temple"] = make_dungeon(
                "Wind Temple",
                item_factory("Wind Temple Big Key", world),
                item_factory(["Wind Temple Small Key"] * 2, world),
                item_factory(["Wind Temple Dungeon Map", "Wind Temple Compass"], world),
            )


def get_dungeon_item_pool(multiworld: MultiWorld) -> list[Item]:
    """
    Retrieve the item pool for all The Wind Waker HD dungeons in the multiworld.

    :param multiworld: The MultiWorld instance.
    :return: List of dungeon items across all The Wind Waker HD dungeons.
    """
    return [
        item for world in multiworld.get_game_worlds("The Wind Waker HD") for item in get_dungeon_item_pool_player(world)
    ]


def get_dungeon_item_pool_player(world: "TWWHDWorld") -> list[Item]:
    """
...
def get_unfilled_dungeon_locations(multiworld: MultiWorld) -> list[Location]:
    """
    Retrieve all unfilled The Wind Waker HD dungeon locations in the multiworld.

    :param multiworld: The MultiWorld instance.
    :return: List of unfilled The Wind Waker HD dungeon locations.
    """
    return [
        location
        for world in multiworld.get_game_worlds("The Wind Waker HD")
...
def modify_dungeon_location_rules(multiworld: MultiWorld) -> None:
    """
    Modify the rules for The Wind Waker HD dungeon locations based on specific player-requested constraints.

    :param multiworld: The MultiWorld instance.
    """
    localized: set[tuple[int, str]] = set()
    dungeon_specific: set[tuple[int, str]] = set()
    for subworld in multiworld.get_game_worlds("The Wind Waker HD"):
            ...
            for location in locations:
                if dungeon_specific:
                    ...
                    if location.name == "Dragon Roost Cavern - First Room":
                        add_item_rule(
                            location,
                            lambda item: item.name != "Dragon Roost Cavern Big Key"
                            or (item.player, "Dragon Roost Cavern Small Key") in dungeon_specific,
                        )
...
def fill_dungeons_restrictive(multiworld: MultiWorld) -> None:
    """
    Correctly fill The Wind Waker HD dungeons in the multiworld.
