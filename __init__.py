...
def run_client(*args: str) -> None:
    """
    Launch the The Wind Waker HD client.
...
components.append(
    Component(
        "The Wind Waker HD Client",
        func=run_client,
        component_type=Type.CLIENT,
        file_identifier=SuffixIdentifier(".aptwwhd"),
        icon="The Wind Waker HD",
    )
)
icon_paths["The Wind Waker HD"] = "ap:worlds.twwhd/assets/icon.png"


class TWWHDContainer(APPlayerContainer):
    """
    This class defines the container file for The Wind Waker HD.
    """

    game: str = "The Wind Waker HD"
    patch_file_ending: str = ".aptwwhd"
...
class TWWHDWeb(WebWorld):
    """
    This class handles the web interface for The Wind Waker HD.
    ...
    theme = "ocean"
    options_presets = twwhd_options_presets
    option_groups = twwhd_option_groups
    rich_text_options_doc = True


class TWWHDWorld(World):
    """
    ...
    options_dataclass = TWWHDOptions
    options: TWWHDOptions

    game: ClassVar[str] = "The Wind Waker HD"
    topology_present: bool = True

    item_name_to_id: ClassVar[dict[str, int]] = {
        name: TWWHDItem.get_apid(data.code) for name, data in ITEM_TABLE.items() if data.code is not None
    }
    location_name_to_id: ClassVar[dict[str, int]] = {
        name: TWWHDLocation.get_apid(data.code) for name, data in LOCATION_TABLE.items() if data.code is not None
    }
    ...
    web: ClassVar[TWWHDWeb] = TWWHDWeb()
    ...
    def _determine_item_classification_overrides(self) -> None:
        ...
        if not options.progression_big_octos_gunboats:
            override_as_useful.append("Progressive Quiver")
    ...
    def _determine_progress_and_nonprogress_locations(self) -> tuple[set[str], set[str]]:
        ...
        def add_flag(option: Toggle, flag: TWWHDFlag) -> TWWHDFlag:
            return flag if option else TWWHDFlag.ALWAYS

        options = self.options

        enabled_flags = TWWHDFlag.ALWAYS
        enabled_flags |= add_flag(options.progression_dungeons, TWWHDFlag.DUNGEON | TWWHDFlag.BOSS)
        enabled_flags |= add_flag(options.progression_tingle_chests, TWWHDFlag.TNGL_CT)
        enabled_flags |= add_flag(options.progression_dungeon_secrets, TWWHDFlag.DG_SCRT)
        enabled_flags |= add_flag(options.progression_puzzle_secret_caves, TWWHDFlag.PZL_CVE)
        enabled_flags |= add_flag(options.progression_combat_secret_caves, TWWHDFlag.CBT_CVE)
        enabled_flags |= add_flag(options.progression_savage_labyrinth, TWWHDFlag.SAVAGE)
        enabled_flags |= add_flag(options.progression_great_fairies, TWWHDFlag.GRT_FRY)
        enabled_flags |= add_flag(options.progression_short_sidequests, TWWHDFlag.SHRT_SQ)
        enabled_flags |= add_flag(options.progression_long_sidequests, TWWHDFlag.LONG_SQ)
        enabled_flags |= add_flag(options.progression_spoils_trading, TWWHDFlag.SPOILS)
        enabled_flags |= add_flag(options.progression_minigames, TWWHDFlag.MINIGME)
        enabled_flags |= add_flag(options.progression_battlesquid, TWWHDFlag.SPLOOSH)
        enabled_flags |= add_flag(options.progression_free_gifts, TWWHDFlag.FREE_GF)
        enabled_flags |= add_flag(options.progression_mail, TWWHDFlag.MAILBOX)
        enabled_flags |= add_flag(options.progression_platforms_rafts, TWWHDFlag.PLTFRMS)
        enabled_flags |= add_flag(options.progression_submarines, TWWHDFlag.SUBMRIN)
        enabled_flags |= add_flag(options.progression_eye_reef_chests, TWWHDFlag.EYE_RFS)
        enabled_flags |= add_flag(options.progression_big_octos_gunboats, TWWHDFlag.BG_OCTO)
        enabled_flags |= add_flag(options.progression_expensive_purchases, TWWHDFlag.XPENSVE)
        enabled_flags |= add_flag(options.progression_island_puzzles, TWWHDFlag.ISLND_P)
        enabled_flags |= add_flag(options.progression_misc, TWWHDFlag.MISCELL)
    ...
    def create_regions(self) -> None:
        """
        Create and connect regions for the The Wind Waker HD world.
            ...
            for i in range(len(rock_spire_shop_ship_locations)):
                ...
                add_item_rule(
                    ...
                    lambda item, locations=other_locs: (
                        item.game == "The Wind Waker HD"
                        and all(location.item is None or item.name != location.item.name for location in locations)
                    )
                    or (
                        item.game != "The Wind Waker HD"
                        and all(
                            location.item is None or location.item.game == "The Wind Waker HD" for location in locations
                        )
                    ),
                )
    ...
    def generate_output(self, output_directory: str) -> None:
        """
        Create the output APTWWHD file that is used to randomize the game.

        :param output_directory: The output directory for the APTWWHD file.
        """
        multiworld = self.multiworld
        player = self.player
        ...
        apTWWHD = TWWHDContainer(
            path=os.path.join(
                output_directory, f"{multiworld.get_out_file_name_base(player)}{TWWHDContainer.patch_file_ending}"
            ),
            player=player,
    ...
    def create_item(self, name: str) -> TWWHDItem:
        """
        ...
        if name in ITEM_TABLE:
            return TWWHDItem(name, self.player, ITEM_TABLE[name], self.item_classification_overrides.get(name))
    ...
    def fill_slot_data(self) -> Mapping[str, Any]:
        ...
        # Add entrances to `slot_data`. This is the same data that is written to the .apTWWHD file.
        entrances = {
